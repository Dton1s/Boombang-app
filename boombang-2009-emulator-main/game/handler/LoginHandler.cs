...
namespace BoomBang.game.handler
{
    class LoginHandler
    {
        ...
        static void cambiar_contra(SessionInstance Session, string[,] Parameters)
        {
            ...
            if (recuperacion != null)
            {
                ...
                if (datos_user != null)
                {
                    ...
                    client.SetParameter("password", Gcrypt(password));
                    client.ExecuteNonQuery("UPDATE usuarios SET password = @password WHERE id = @user_id");
                    server.AppendParameter(1);
                    Session.SendData(server);
                    client.SetParameter("codigo", int.Parse(Parameters[0, 0]));
                    client.ExecuteNonQuery("DELETE FROM usuarios WHERE codigo = @codigo");

                    email_manager((string)datos_user["email"], "Contraseña", "Guarda tu nueva contraseña y no se la digas a nadie : " + Parameters[1, 0]);
                    return;
                }
            }
            server.AppendParameter(-1);
        ...
        private static string Gcrypt(string contraseña)
        {
        ...
        static void email_manager(string destinatario, string titulo, string body)
        {
            ...
            System.Net.NetworkCredential mailAuthentication = new System.Net.NetworkCredential("ame3596@gmail.com", "asop zkvj yoxr cihm");

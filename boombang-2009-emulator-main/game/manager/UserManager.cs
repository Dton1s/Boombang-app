...
namespace BoomBang.game.manager
{
    class UserManager
    {
        ...
        public static bool Actualizar_Contraseña(UserInstance User, string Password)
        {
            if (User != null)
            {
                using (mysql client = new mysql())
                {
                    ...
                    if (client.ExecuteNonQuery("UPDATE usuarios SET password = @password WHERE id = @id") == 1)
                    {
        ...
        public static void ActualizarVipUsuarios(SessionInstance Session)
        {
            if (Session.User != null)
            {
                ...
                if (Meses_Usuario == 12|| Meses_Usuario == 24 || Meses_Usuario == 36 || Meses_Usuario == 48 || Meses_Usuario == 60)
                {
                    ...
                    FlowerHandler.BoomBangTeam(Session, "¡Felicidades " + Session.User.nombre + " Ahora eres VIP en BoomBang! \r\rTe notificaremos cuando tu suscripción termine faltando 15 días para que puedas renovar tu Membresía.");
                    server.AppendParameter(1);
                    server.AppendParameter(Session.User.end_vip);
                    Session.SendData(server);
